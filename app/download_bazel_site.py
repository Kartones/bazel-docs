...
def clone_repository(repo_path: Path) -> None:
    ...
    run_git_command(["git", "fetch", "--depth", "1", "origin", "master"], cwd=repo_path)
    run_git_command(["git", "checkout", "master"], cwd=repo_path)
...
def update_repository(repo_path: Path) -> None:
    ...
    run_git_command(["git", "fetch", "--depth", "1", "origin", "master"], cwd=repo_path)
    run_git_command(["git", "reset", "--hard", "origin/master"], cwd=repo_path)
...
def main() -> None:
    ...
    repo_path = Path("input/bazel-site")
